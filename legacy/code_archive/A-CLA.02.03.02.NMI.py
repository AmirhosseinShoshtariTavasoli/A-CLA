...
from networkx.algorithms.community import louvain_communities, label_propagation_communities, girvan_newman
...
file_path = 'adjacency_matrix_netscience_output.xlsx'
...
# Convert NetworkX graph to iGraph for leading eigenvector method
...
gn_comms = next(gn_generator)
labels_gn = communities_to_labels(gn_comms, len(G.nodes()))

# Label Propagation method
lp_comms = list(label_propagation_communities(G))
labels_lp = communities_to_labels(lp_comms, len(G.nodes()))

# Leading Eigenvector method using igraph
le_comms = g.community_leading_eigenvector()
labels_le = le_comms.membership
...
# Step 4: Calculate NMI for A-CLA with each method
nmi_acla_louvain = normalized_mutual_info_score(labels_acla, labels_louvain)
nmi_acla_gn = normalized_mutual_info_score(labels_acla, labels_gn)
nmi_acla_lp = normalized_mutual_info_score(labels_acla, labels_lp)
nmi_acla_le = normalized_mutual_info_score(labels_acla, labels_le)

# Print the NMI results
print(f"NMI A-CLA vs Louvain: {nmi_acla_louvain:.4f}")
print(f"NMI A-CLA vs Girvan-Newman: {nmi_acla_gn:.4f}")
print(f"NMI A-CLA vs Label Propagation: {nmi_acla_lp:.4f}")
print(f"NMI A-CLA vs Leading Eigenvector: {nmi_acla_le:.4f}")
